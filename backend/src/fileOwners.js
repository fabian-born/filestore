...
const upsert = db.prepare(
  `INSERT INTO file_owners (object_key, owner_id, owner_username, size, content_type, created_at) VALUES (?, ?, ?, ?, ?, ?)
...
// usage can be summed without a bucket listing; content_type likewise, for
// the file-type breakdown stat.
export function setFileOwner(objectKey, ownerId, ownerUsername, size, contentType) {
  upsert.run(objectKey, ownerId, ownerUsername, size || 0, contentType || null, new Date().toISOString());
}
...
export function getFileOwners(objectKeys) {
  ...
  const rows = db
    .prepare(
      `SELECT object_key, owner_id, owner_username, size, content_type FROM file_owners WHERE object_key IN (${objectKeys.map(() => '?').join(',')})`
    )
    .all(...objectKeys);
  return new Map(
    rows.map((r) => [
      r.object_key,
      { ownerId: r.owner_id, ownerUsername: r.owner_username, size: r.size, contentType: r.content_type },
    ])
  );
}
